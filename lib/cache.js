 ...
 */
  static getUiInfo() {
    if (Env.get('uiInfo')) return Env.get('uiInfo');
    const { ui_location } = sysEnv();
    let base = process.env.ui_home || ui_location;
    let app_file = join(base, "app", "index.json");
    if (existsSync(app_file)) {
      Env.set('uiInfo', readFileSync(app_file));
      console.log("AAA:85", Env.get('uiInfo'))
      return Env.get('uiInfo')
    }
    Env.set('uiInfo', {});
    return Env.get('uiInfo');
  }
