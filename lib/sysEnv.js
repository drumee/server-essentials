...
function load(chroot) {
  ...
  let server_location = join(server_dir, endpoint_name);
  ...
  let legacy_ui = join(ui_base, 'dist', endpoint_name);
  if (existsSync(legacy_ui)) {/** Legacy  */
    ui_location = legacy_ui;
  }

  let legacy_srv = join(server_dir, 'dist', endpoint_name);
  if (existsSync(legacy_srv)) {
    server_location = legacy_srv;
  }
  ...
  COFIGS = {
    ...
    runtime_dir,
    server_dir,
    server_home: env.server_home || server_dir,
    server_location,
    socketPath,
    static_dir,
    svc_location,
...
function getSysConst(attr, value) {
  if (CONST.get(attr) == null) {
    console.warn(`${attr} already set. Ignoring new value`);
    return
  }
...
function loadUinfo() {
  let { ui_home } = sysEnv();
  let app_file = join(ui_home, "app", "index.json");
  if (existsSync(app_file)) {
    UI_INFO = readFileSync(app_file);
  }
}
...
module.exports = { sysEnv, getSysConst, setSysConst, loadSysEnv: load, getUiInfo };
